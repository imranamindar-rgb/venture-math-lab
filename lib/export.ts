...
export function buildScenarioCsv(config: ScenarioConfig, provided?: SimulationSummary) {
  ...
  const rows: Array<Array<string | number>> = [
    ...
    [
      "Simulation",
      "Founder median 95% stability range",
    ...
    [
      "Simulation",
      "Employee underwater 95% stability range",
...
export function buildScenarioMarkdown(config: ScenarioConfig, provided?: SimulationSummary) {
  ...
  return [
    ...
    `- Founder median 95% stability range: ${formatCurrency(payload.simulation.confidence.founderMedian.lower)} to ${formatCurrency(payload.simulation.confidence.founderMedian.upper)}`,
    `- Employee underwater: ${formatPercent(payload.simulation.employee.underwaterProbability)}`,
    `- Employee underwater 95% stability range: ${formatPercent(payload.simulation.confidence.employeeUnderwaterProbability.lower)} to ${formatPercent(payload.simulation.confidence.employeeUnderwaterProbability.upper)}`,
    `- Investor return-the-fund: ${formatPercent(payload.simulation.investor.returnTheFundProbability)}`,
    `- Investor return-the-fund 95% stability range: ${formatPercent(payload.simulation.confidence.investorReturnTheFundProbability.lower)} to ${formatPercent(payload.simulation.confidence.investorReturnTheFundProbability.upper)}`,
