import { scenarioPresets, stagePresets, getStageIndex, marketOverlayMultipliers, sectorOverlayMultipliers } from "@/data/presets";
import {
...
import { issuePreferredRound, maybeRefreshPool, sampleNextRound, getReferencePostMoney } from "@/lib/sim/rounds";
import {
  FundingStage,
  HistogramBucket,
  OwnershipPoint,
...
import { applySecondaryLiquidity } from "@/lib/sim/cap-table";
...
function summarizeStakeholder(values: number[]): StakeholderSummary {
  ...
  const riskBand =
    medianValue < 250_000 ? "Critical" : medianValue < 1_000_000 ? "Tight" : p90 > medianValue * 12 ? "Power-Law" : "Healthy";
...
function buildSummary(config: ScenarioConfig, paths: PathOutcome[]): SimulationSummary {
  ...
  return {
    ...
    warnings: [
      ...config.warningFlags,
      "v1 uses standard-friendly preferred stock assumptions and does not model bespoke anti-dilution or warrant structures.",
