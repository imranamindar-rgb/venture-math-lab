...
function normalizedBasePercents(config: ScenarioConfig) {
  ...
  return {
    founder: config.capTable.founderPercent / safeTotal,
...
export function createInitialCapTable(config: ScenarioConfig): CapTableSnapshot {
  ...
  return {
    ...
    modeledInvestorInvested:
      config.currentRoundKind === "priced_preferred" ? config.investor.initialCheck : config.investor.initialCheck,
